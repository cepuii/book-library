...
public class LoanUtil {
    ...
    public static final LoanTO LOAN_TO = LoanTO.builder()
            ...
            .bookId(BOOK_ID)
            .userId(USER_ID)
            .startDate(Date.from(START_DATE.atStartOfDay(ZoneId.systemDefault()).toInstant()))
            .endDate(Date.from(START_DATE.plusDays(DURATION).atStartOfDay(ZoneId.systemDefault()).toInstant()))
            .status(STATUS.name())
            .bookInfo(BOOK_INFO)
            .fine(FINE)
            .build();
